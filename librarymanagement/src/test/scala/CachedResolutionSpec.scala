...
class CachedResolutionSpec extends BaseIvySpecification {
  ...
  "Resolving the same module twice" should "work" in {
    cleanIvyCache()
    val m = module(
      new ModuleID("com.example", "foo", "0.1.0").withConfigurations(xsbti.Maybe.just("compile")),
  ...
  "Resolving the unsolvable module should" should "not work" in {
    ...
    val m = module(
      new ModuleID("com.example", "foo", "0.2.0").withConfigurations(xsbti.Maybe.just("compile")),
  ...
  "Resolving a module with a pseudo-conflict" should "work" in {
    ...
    cleanIvyCache()
    val m = module(
      new ModuleID("com.example", "foo", "0.3.0").withConfigurations(xsbti.Maybe.just("compile")),
    ...
    // second resolution reads from the minigraph
    val report = ivyUpdate(m)
    val modules: Seq[String] = report.configurations.head.modules flatMap { _.lines }
  ...
  def mavenCayennePlugin302 = new ModuleID("org.apache.cayenne.plugins", "maven-cayenne-plugin", "3.0.2").withConfigurations(xsbti.Maybe.just("compile"))
  def avro177 = new ModuleID("org.apache.avro", "avro", "1.7.7").withConfigurations(xsbti.Maybe.just("compile"))
  def dataAvro1940 = new ModuleID("com.linkedin.pegasus", "data-avro", "1.9.40").withConfigurations(xsbti.Maybe.just("compile"))
  def netty320 = new ModuleID("org.jboss.netty", "netty", "3.2.0.Final").withConfigurations(xsbti.Maybe.just("compile"))
