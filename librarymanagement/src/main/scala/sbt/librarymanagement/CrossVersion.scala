...
object CrossVersionUtil {
   ...
   */
  def fullMapped(remapVersion: String => String): CrossVersion = new Full(f1(remapVersion))
  ...
  def apply(module: ModuleID, is: IvyScala): Option[String => String] =
    CrossVersionUtil(module.crossVersion, is.scalaFullVersion, is.scalaBinaryVersion)
  ...
  private[sbt] def substituteCross(exclude: SbtExclusionRule, is: Option[IvyScala]): SbtExclusionRule = {
    val fopt: Option[String => String] =
      is flatMap { i => CrossVersionUtil(exclude.crossVersion, i.scalaFullVersion, i.scalaBinaryVersion) }
