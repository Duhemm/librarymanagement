...
final class DeliverConfiguration(val deliverIvyPattern: String, val status: String, val configurations: Option[Seq[Configuration]], val logging: UpdateLogging)
final class PublishConfiguration(val ivyFile: Option[File], val resolverName: String, val artifacts: Map[Artifact, File], val checksums: Seq[String], val logging: UpdateLogging,
  val overwrite: Boolean) {
  def this(ivyFile: Option[File], resolverName: String, artifacts: Map[Artifact, File], checksums: Seq[String], logging: UpdateLogging) =
    this(ivyFile, resolverName, artifacts, checksums, logging, false)
}

// final class UpdateConfiguration(val retrieve: Option[RetrieveConfiguration], val missingOk: Boolean, val logging: UpdateLogging, val artifactFilter: ArtifactTypeFilter) {
//   @deprecated("You should use the constructor that provides an artifactFilter", "1.0.x")
//   def this(retrieve: Option[RetrieveConfiguration], missingOk: Boolean, logging: UpdateLogging) {
...
//     logging: UpdateLogging = this.logging,
...
object IvyActions {
  ...
  private def crossVersionMap(moduleSettings: ModuleSettings): Option[String => String] =
    moduleSettings match {
      case i: InlineConfiguration => CrossVersionUtil(i.module, m2o(i.ivyScala))
      case _                      => None
    }
        ...
        ivy.getResolveEngine match {
          case x: CachedResolutionResolveEngine =>
            ...
            x.customResolve(md, configuration.missingOk, logicalClock, resolveOptions, depDir getOrElse { sys.error("dependency base directory is not specified") }, log) match {
              ...
              case Right(uReport) =>
                m2o(configuration.retrieve) match {
      ...
      updateEither(module, upConf, uwconfig, logicalClock, depDir, log) match {
        case Right(r) =>
          ...
          val typeClassifierMap: Map[String, String] =
            ((sourceArtifactTypes.toIterable map (_ -> ArtifactUtil.SourceClassifier))
              :: (docArtifactTypes.toIterable map (_ -> ArtifactUtil.DocClassifier)) :: Nil).flatten.toMap
          r.substitute { (conf, mid, artFileSeq) =>
            artFileSeq map {
              case (art, f) =>
                // Deduce the classifier from the type if no classifier is present already
                art.withClassifier(o2m(m2o(art.classifier) orElse typeClassifierMap.get(art.tpe))) -> f
            }
          }
        case Left(w) =>
          throw w.resolveException
  ...
  def classifiedArtifacts(name: String, classifiers: Seq[String]): Seq[Artifact] =
    classifiers map { c => ArtifactUtil.classified(name, c) }
  ...
  private def retrieve(log: Logger, ivy: Ivy, report: UpdateReport, config: RetrieveConfiguration): UpdateReport = {
    ...
    retrieve(log, ivy, report, config.retrieveDirectory, config.outputPattern, config.sync, m2o(config.configurationsToRetrieve).map(_.asScala.toSet))
  }
